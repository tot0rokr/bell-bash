---
name: bell
description: Send a rich notification via `bell-send` (terminal BEL + desktop toast + optional Slack/Discord webhook). Use for long-running task completions (build, test, deploy, migration, big refactor — anything ≥30s of wall-clock work), to flag a failure with critical urgency, or whenever a custom title/body adds context the user wants. Trigger phrases include "알림 켜줘", "끝나면 알려줘", "ping me when done", "notify when ready", "send a finish alert".
---

# bell — 풍부한 작업 완료 알림

이 skill은 `bell-send` 로 컨텍스트가 실린 알림 — 장시간 작업 완료(Slack/Discord webhook 포함), 실패 신호(`--status` 로 critical urgency), 사용자가 요청한 커스텀 제목/본문 — 을 보낼 때 호출한다.
